...
public class HanaDatabase extends AbstractJdbcDatabase {
    ...
    public void setConnection(DatabaseConnection conn) {
        if (conn instanceof JdbcConnection) {
            ...
            if (connection == null) {
                Scope.getCurrentScope().getLog(getClass()).info(LogType.LOG, "Could not get JDBC connection");
            } else {
                try {
                    addReservedWords(Arrays.asList(connection.getMetaData().getSQLKeywords().split(",\\s*")));
                } catch (SQLException e) {
                    Scope.getCurrentScope().getLog(getClass()).info(LogType.LOG, "Could not get SQL keywords: " + e.getMessage());
                }
                ...
                try (PreparedStatement statement = connection
                    ...
                    Scope.getCurrentScope().getLog(getClass()).info(LogType.LOG, "Could not get system views: " + e.getMessage());
                }
                ...
                try (PreparedStatement statement = connection
                    ...
                    Scope.getCurrentScope().getLog(getClass()).info(LogType.LOG, "Could not get system tables: " + e.getMessage());
                }
            }
        }
