...
class ParserFactory_XmlChangeLogTest extends Specification {

    @Unroll("#featureName: #description")
    def "can parse changelog"() {
        when:
        def resourceAccessor = new MockResourceAccessor()
                .addResource("com/example/test.xml", xml.trim())

        def changelog = null
        Scope.child(Scope.Attr.resourceAccessor, resourceAccessor, {
            changelog = Scope.currentScope.getSingleton(ParserFactory).parse("com/example/test.xml", ChangeLog)
        })

        then:
        changelog.describe() == expected

        where:
        [description, xml, expected] << [
                [
                        "Empty changelog",
                        """
...
""",
                        "ChangeLog{physicalPath=com/example/test.xml}"
                ],
                [
                        "Single changeset",
                        """
...
""",
                        "ChangeLog{changeLogEntries=[null::1::bob], physicalPath=com/example/test.xml}" //todo: better check when changes are converted to actions
                ]
        ]
