...
public class XmlParser_RealFile_Test extends Specification {
    ...
    def "namespace configured correctly"() {
        expect:
        assert new XmlParser().saxParserFactory.isNamespaceAware()
        assert new XmlParser().saxParserFactory.isValidating()
    }
    ...
    def "able to parse a simple changelog simpleChangeLog.xml"() throws Exception {
        ...
        when:
        def changeLog = new XmlParser().parse(path, new ChangeLogParameters(), new JUnitResourceAccessor());
    ...
    def "local path can be set in changelog file logicalPathChangeLog.xml"() throws Exception {
        ...
        def changeLog = new XmlParser().parse(physicalPath, new ChangeLogParameters(), new JUnitResourceAccessor())
    ...
    def "changelog with preconditions can be parsed: preconditionsChangeLog.xml"() throws Exception {
        ...
        def changeLog = new XmlParser().parse(path, params, new JUnitResourceAccessor());
    ...
    def "ChangeLogParseException thrown if changelog does not exist"() throws Exception {
        ...
        new XmlParser().parse(path, new ChangeLogParameters(), new JUnitResourceAccessor())
    ...
    def "ChangeLogParseException thrown if changelog has invalid tags"() throws Exception {
        when:
        new XmlParser().parse("liquibase/parser/core/xml/malformedChangeLog.xml", new ChangeLogParameters(), new JUnitResourceAccessor())
    ...
    def "ChangeLogParseException thrown if changelog is invalid XML: invalidChangeLog.xml"() throws Exception {
        when:
        new XmlParser().parse("liquibase/parser/core/xml/invalidChangeLog.xml", new ChangeLogParameters(), new JUnitResourceAccessor())
	...
	def "changeLog parameters are correctly expanded"() throws Exception {
		...
		def changeLog = new XmlParser().parse("liquibase/parser/core/xml/parametersChangeLog.xml", params, new JUnitResourceAccessor());
    ...
    def "change sets with matching dbms are parsed"() {
        ...
        def changeLog = new XmlParser().parse(path, new ChangeLogParameters(database), new JUnitResourceAccessor())
