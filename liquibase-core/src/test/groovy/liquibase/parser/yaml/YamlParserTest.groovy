...
class YamlParserTest extends Specification {
    ...
    def "can parse a simple file"() {
        ...
        then:
        parsedNode.prettyPrint() == expected.trim()
...
""",
    ...
    @Unroll
    def "invalid yaml throws an error"() {
        when:
        def parsedNode = null
        def resourceAccessor = new MockResourceAccessor()
                .addResource("com/example/test.yaml", yaml)
        ...
        then:
        def e = thrown(ParseException)
        e.message.trim() == expected.trim()

        where:
        [yaml, expected] << [
                [
                        "rootNode",
                        "Syntax error in com/example/test.yaml: Can't construct a java object for tag:yaml.org,2002:java.util.Map; exception=No single argument constructor found for interface java.util.Map : null\n in 'reader', line 1, column 1:\n    rootNode\n    ^",
                ],
                [
                        """
rootNode
 ...
 indent2: false
""",
                        "Syntax error in com/example/test.yaml: mapping values are not allowed here\n in 'reader', line 3, column 10:\n      indent1: true\n             ^"
                ]
        ]
    }
...
//        new YamlParser().parse("com/example/test.yaml")
//
//        then:
//        def e = thrown(ParseException)
