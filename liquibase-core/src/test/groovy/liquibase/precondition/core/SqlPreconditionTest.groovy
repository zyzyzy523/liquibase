...
class SqlPreconditionTest extends Specification {
    ...
    def "load works with nested sql"() {
        ...
        try {
            precondition.load(new liquibase.parser.ParsedNode(null, "sqlCheck").addChild(null, "expectedResult", "5").setValue("select count(*) from test"), resourceSupplier.simpleResourceAccessor)
